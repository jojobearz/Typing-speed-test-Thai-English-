...
FingerMap[7] = "Right Ring (¹ÔéÇªÕé¢ÇÒ)";
...
function calculateAverageTypedKeySpeed()
{
  ...
  for (var i = 1; i < TypedKeysTime.length; i++)
  {
    ...
    //alert(AverageTypedKeySpeed[ "key" + TypedKeys[i] ] + "," + numTimesPressed[ "key" + TypedKeys[i] ]);
  }
...
function endTest()
{
  ...
  var fingerSpeedTableHeaderRow = "<tr><td style=\"text-align:center;background-color:#C0C0C0;color:white;font-weight:bold;line-height:1.3;\" colspan=\"2\">Average Finger Response Time</td></tr>";
  ...
  for (var i = 0; i <= 8; i++)
  {
    if (FingerUsage[i].toFixed() == 0) {
      ...
      fingerSpeedRows[i] = "<tr><td style=\"color:black;text-align:left\">" + FingerMap[i] + ": </td><td style=\"color:black;text-align:right\"> " + FingerUsage[i].toFixed() + " Milliseconds</td></tr>";
    }
  }
...
function configResetTestSettings()
{
  ...
  if (document.getElementById("lstLanguageInput").value == "thai") {
	KeyboardLayoutMap[0] = new Array();
	KeyboardLayoutMap[0][0] = "% +åñ/ò-óÀô¶ÙØßÖõ¤öµ÷¨ø¢ùª";
